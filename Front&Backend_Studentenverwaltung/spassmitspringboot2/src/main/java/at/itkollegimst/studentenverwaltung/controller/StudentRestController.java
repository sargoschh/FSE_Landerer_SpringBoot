...
/*
 * Wir entwickeln das Frontend mit JavaScript - JavaScript Application wird
 * dann lokal auf einem zweiten Server hochgefahren. Es werden dann auf
 * diesem zweiten Server Requests an den ersten Server (zum Backend)
 * gesendet - dies wird als CrossOrigin-Request bezeichnet. Sprich,
 * derjenige, der die API anfragt, liegt nicht am selben Server
 * wie derjenige, der die API ausliefert. Dies wird standardmäßig von
 * Browsern nicht erlaubt, außer wenn das Backend durch einen entsprechenden
 * Header signalisiert, dass von einer bestimmten Origin (Quelle der Anfrage)
 * aus diese Anfrage auch erlaubt werden soll.
 */
